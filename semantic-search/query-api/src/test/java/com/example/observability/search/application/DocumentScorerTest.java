...
class DocumentScorerTest {
  ...
  @Test
  void missingCandidateFieldsDoNotEarnQueryTokenBoosts() {
    SearchCriteria criteria = new SearchCriteria("database latency after deployment",
        "trading", "production", Instant.parse("2026-07-01T08:00:00Z"), Instant.parse("2026-07-01T10:30:00Z"), 5,
        SearchMode.HYBRID);
    SearchCandidate candidate = new SearchCandidate("1", "incident", "trading", "production",
        Instant.parse("2026-07-01T10:10:00Z"), "", "WARN", null,
        "Risk-service database pool exhaustion.", "semantic text",
        List.of(""), List.of(new EvidenceLink("trace", "grafana", "https://grafana.example/tempo")), 1.0,
        "keyword");

    List<ScoredResult> results = scorer.score(criteria, List.of(candidate), List.of());

    assertThat(results.getFirst().explanation()).doesNotContain("same service", "topology proximity");
  }
}
